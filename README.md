...
*   **API 密钥管理**:
    ...
    *   提供 API 接口用于检查密钥状态，并支持手动批量删除异常密钥。
...
*   **一键检查**： 点击“一键检查”按钮，可以检查 API key 可用性；检查失败的 key 会被标记为异常，不会自动删除。
*   **批量删除**： 选中异常的 API key，可以一键删除所有异常的 API key。
...
*   `POST /api/keys/check`: 检查密钥有效性并更新状态。请求体为 `{"keys": ["key1", "key2"]}`。
